        ...
        >
          ...
          <div class="absolute bottom-0 left-0 w-full p-6 text-white">
            <div class="max-w-4xl mx-auto">
              <h2 class="text-3xl sm:text-4xl font-bold mb-2">{{ anime.title }}</h2>
              <p class="text-sm sm:text-base mb-4 line-clamp-2">{{ anime.description }}</p>
              <div class="flex items-center space-x-3 mb-4">
                <div class="bg-blue-500 px-2 py-1 rounded text-xs">{{ anime.status === 'ongoing' ? '連載中' : anime.status === 'upcoming' ? '即將播出' : '已完結' }}</div>
                <div class="text-amber-400 flex items-center text-sm">
                  ⭐ {{ anime.rating }}
                </div>
                <div class="text-sm">{{ anime.genres.join(' / ') }}</div>
              </div>
    ...
    <div class="container mx-auto px-4">
      ...
      <section class="mb-10">
        <div class="flex justify-between items-center mb-4">
          ...
          <RouterLink to="/bangumi" class="text-blue-600 hover:text-blue-800 text-sm">
            查看全部 →
          </RouterLink>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          >
            <RouterLink :to="`/watch?id=${episode.bangumiId}&episode=${episode.id}`">
              <div class="relative">
                <div class="aspect-video bg-stone-200"></div>
                <div class="absolute bottom-2 right-2 bg-black bg-opacity-70 text-white text-xs px-1 rounded">
                  {{ episode.duration }}
                </div>
              </div>
              <div class="p-3">
                <div class="font-medium line-clamp-1">{{ getBangumiTitle(episode.bangumiId) }}</div>
                <div class="text-sm text-stone-500 mt-1">第{{ episode.number }}集：{{ episode.title }}</div>
                <div class="text-xs text-stone-400 mt-1">{{ episode.airDate }}</div>
              </div>
            </RouterLink>
          </div>
        </div>
      ...
      <section class="mb-10">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-2xl font-bold">本季新番</h2>
          <RouterLink to="/bangumi" class="text-blue-600 hover:text-blue-800 text-sm">
            查看全部 →
          </RouterLink>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          ...
          >
            <div class="bg-white rounded-lg shadow overflow-hidden transition group-hover:shadow-md">
              ...
              <div class="p-3">
                <div class="font-medium line-clamp-1 group-hover:text-blue-600 transition">{{ anime.title }}</div>
      ...
      <!-- 熱門排行榜 -->
      <section class="mb-10">
        <h2 class="text-2xl font-bold mb-4">人氣排行榜</h2>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <!-- 左側：熱門番劇 -->
          <div>
            <h3 class="text-lg font-semibold mb-3">熱門番劇 TOP 5</h3>
            <div class="bg-white rounded-lg shadow overflow-hidden">
              ...
              >
                <div class="font-bold text-xl text-stone-400 w-8">{{ index + 1 }}</div>
                <img 
                  :src="anime.image" 
                  :alt="anime.title" 
          ...
          <!-- 右側：熱門劇集 -->
          <div>
            <h3 class="text-lg font-semibold mb-3">熱門劇集 TOP 5</h3>
            <div class="bg-white rounded-lg shadow overflow-hidden">
              <RouterLink 
              ...
              >
                <div class="font-bold text-xl text-stone-400 w-8">{{ index + 1 }}</div>
                <div class="w-16 aspect-video bg-stone-200 rounded flex-shrink-0"></div>
                <div class="ml-3 flex-1">
                  <div class="font-medium line-clamp-1">{{ getBangumiTitle(episode.bangumiId) }}</div>
                  <div class="text-xs text-stone-500 mt-1">第{{ episode.number }}集 • {{ episode.views.toLocaleString() }} 次觀看</div>
                </div>
              </RouterLink>
            </div>
          </div>
      ...
      <!-- 繼續觀看 (登錄用戶) -->
      <section v-if="recentlyWatched.length" class="mb-10">
        <h2 class="text-2xl font-bold mb-4">繼續觀看</h2>
          ...
          >
            <RouterLink :to="`/watch?id=${anime.id}`">
              ...
              <div class="p-3">
                ...
                <div class="text-xs text-stone-500 mt-1">繼續觀看</div>
              </div>
            </RouterLink>
          </div>
        </div>
      ...
      <section class="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl p-6 mb-10 text-white">
        <div class="max-w-3xl mx-auto text-center">
          <h2 class="text-2xl font-bold mb-3">獲取最新番劇更新通知</h2>
          <p class="mb-6 opacity-90">訂閱我們的通知，第一時間獲取新番更新和熱門推薦</p>
